...
import { useGetStatementMutation } from "../../slices/production/packHousePeopleApiSlice";
...
const PackhousePeopleReportScren = () => {
  ...
  const { data: packPeople } = useGetAllPackHousePeopleQuery();
  const [packHousePeople] = useGetStatementMutation();
  ...
  const loaddata = async () => {
    ...
    const data = await packHousePeople({
      report_name,
      start_date,
      end_date,
    }).unwrap();
          ...
          <Col>
            <Form.Group className="my-2" controlId="role_name">
              <Form.Select
                ...
                value={report_name}
                onChange={(e) => set_report_name(parseInt(e.target.value))}
              >
                <option value="">Select Person</option>
                {packPeople?.data.map((item, index) => (
...
export default PackhousePeopleReportScren;
