...
import { useGeneralDailyPackbypacktypeMutation } from "../../slices/production/dailyPackhouseHeadersApiSlice";
...
const DailyPackHouseReportScreen = () => {
  ...
  const [
    setGeneralDailyPackbypacktype,
    { isLoading: isLoading1, isSuccess: isSuccess1, isError: isError1 },
  ...
  const loaddata = async () => {
    ...
    const data = await setGeneralDailyPackbypacktype({
      report_name,
      start_date,
      end_date,
    }).unwrap();
  ...
  const columns1 = useMemo(
    () => [
      ...
      { Header: "Number", accessor: "total_number_packed" },
      ...
      {getData && getData.length > 0 ? (
        <>
          ...
          <Row>
            {" "}
            <DataTable
              columns={report_name == "detailed" ? columns : columns1}
              data={getData}
            />
          </Row>
        </>
...
export default DailyPackHouseReportScreen;
