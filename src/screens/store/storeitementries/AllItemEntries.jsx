...
const AllItemEntries = () => {
  ...
  const { data: items, isLoading } = useGetAllStoreItemEntriesQuery();
  ...
  const handleDownloadPDF = async (next) => {
    ...
    try {
      const response = await axios({
        url: `${baseUrlJasper}/all/store/items/entries/pdf`, // Endpoint on your Node.js server
        method: "GET",
      ...
      link.href = url;
      link.setAttribute("download", "all-store-items-entries-report.pdf");
  ...
  const handleDownloadExcel = async () => {
    ...
    try {
      const response = await axios({
        url: `${baseUrlJasper}/all/store/items/entries/excel`, // Endpoint on your Node.js server
        method: "GET",
        responseType: "blob", // Important: responseType 'blob' for binary data
      });
      ...
      const link = document.createElement("a");
      link.href = url;
      link.setAttribute("download", "all-store-items-entries-report.xlsx");
      document.body.appendChild(link);
      link.click();
      window.URL.revokeObjectURL(url);
    } catch (error) {
